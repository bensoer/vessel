...
def generate_private_key(password:str)->bytes:
...
def generate_public_key(encrypted_private_key:bytes, password:str)->bytes:
    '''
...
def encrypt_bytes_with_aes_key_to_base64_bytes(plaintext_bytes:bytes, aes_key:bytes)->bytes:
    ...
    padding_data = (AES.block_size - len(plaintext_bytes) % AES.block_size) * chr(AES.block_size - len(plaintext_bytes) % AES.block_size)

    padded_message = plaintext_bytes + padding_data.encode()
...
def decrypt_base64_bytes_with_aes_key_to_bytes(base64_cipher_bytes:bytes, aes_key:bytes)->bytes:
    ...
    encrypted_message_with_iv = base64.b64decode(base64_cipher_bytes)
...
def encrypt_bytes_with_public_key_to_base64_bytes(plaintext_bytes:bytes, exported_public_key:bytes)->bytes:

    public_key = RSA.import_key(exported_public_key)
    cipher_rsa = PKCS1_OAEP.new(public_key, hashAlgo=libsha256)

    encrypted_bytes_message = cipher_rsa.encrypt(plaintext_bytes)
...
def decrypt_base64_bytes_with_private_key_to_bytes(base64_cipher_bytes:bytes, exported_private_key:bytes, private_key_password:str)->bytes:
    '''
